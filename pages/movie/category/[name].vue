...
const { data, pending: isLoaded } = await useFetch<APIResponse>(`/api${name === 'trending' ? '/trending' : ''}/movie`, {
  params: {
...
<template>
  <main relative my-8 xl:ml-25>
    <Listing
      v-if="items && items.results.length" title="Trending Movies" :loading="loading" :items="items"
      @load-more="loadMore"
    />
  </main>
</template>
